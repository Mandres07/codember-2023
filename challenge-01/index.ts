...
let resObj: Sorted[] = [];
list.forEach(item => {
   const itemIndex = resObj.findIndex(li => li.value === item);
   if (itemIndex < 0)
      resObj.push({ value: item, quantity: 1 });
   else 
      resObj[itemIndex].quantity++;
});
